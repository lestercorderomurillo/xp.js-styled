...
const StyledView = (0, createStyled_1.createStyled)(react_native_1.View);
const ColorSample = () => {
    const lumens = [];
    ...
    return (<react_native_1.FlatList numColumns={17} data={colorLumens} keyExtractor={(item) => `${item.colorName}-${item.lumen}`} renderItem={({ item, index }) => (<StyledView height={25} width={25} backgroundColor={(0, transformers_1.color)(`${item.colorName}.${item.lumen}`)} style={{
