...
exports.size = exports.color = exports.shade = exports.media = exports.deepMerge = exports.deepTransform = exports.deepColor = exports.deepSize = exports.deepMap = exports.splitProps = void 0;
 ...
 */
const deepMap = ({ input, context, match, map, skipKeys }) => {
    const shouldSkip = (key) => skipKeys.includes(key);
    if (match(input)) {
        ...
        for (const key in input) {
            if (!shouldSkip(key)) {
                ...
                if (match(value)) {
                    ...
                    output[key] = (0, exports.deepMap)({ input: value, context, match, map, skipKeys });
                }
            }
        }
        return output;
...
const media = (values, breakpoints) => {
    ...
    if (values && Object.keys(values).length > 0) {
        ...
        if (platformSpecificValue) {
            output = (0, exports.deepMerge)([output, platformSpecificValue]);
        }
        for (const breakpointKey of breakpointKeys) {
            ...
            if (width > threshold && value) {
                output = (0, exports.deepMerge)([output, value]);
            }
        }
        if (colorScheme == "dark" && values["@dark"]) {
            output = (0, exports.deepMerge)([output, values["@dark"]]);
        }
        else if (colorScheme == "light" && values["@light"]) {
            output = (0, exports.deepMerge)([output, values["@light"]]);
        }
    }
    return output;
};
